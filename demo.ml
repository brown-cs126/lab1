...
let rec length : int list -> int = fun ns ->
    match ns with
    | [] -> 0
    | _ :: ns ->
        1 + length ns

let rec count_down : int -> int list = fun n ->
    if (n = 0) then [0]
    else n :: count_down (n - 1)

let rec sum : int list -> int = fun ns ->
    match ns with
    | [] -> 0
    | n :: ns ->
        n + sum ns

let rec product : int list -> int = fun ns ->
    match ns with
    | [] -> 1
    | n :: ns ->
        n * product ns
...
= 0
 *)
let rec product_helper : int -> int list -> int = fun p ns ->
    match ns with
    | [] -> p
    | 0 :: _ -> 0
    | n :: ns -> product_helper (p * n) ns
...
let rec fib_helper : int -> int -> int -> int = fun prev cur n ->
    if (n = 0) then cur
    else fib_helper cur (prev + cur) (n - 1)
...
let _ = assert (["0a"; "1b"; "2c"] =
    List.mapi (fun i s -> string_of_int i ^ s) ["a"; "b"; "c"])

let range : int -> int list = fun n -> List.rev (count_down n)

let _ = assert ([3;2;1;0;2;1;0;1;0;0] =
    List.concat_map count_down (count_down 3))
...
let _ = assert ("-123" =
    List.fold_left (fun s n -> s ^ string_of_int n) "-" [1;2;3])
...
let _ = assert (rc.name = "Doug"); assert (rc.course = 1260)
let {name; course} = rc
let _ = assert (name = "Doug"); assert (course = 1260)

let tp : (string * int) = ("Doug", 1260)
let (name, course_number) = tp
let _ = assert (name = "Doug"); assert (course_number = 1260);;
